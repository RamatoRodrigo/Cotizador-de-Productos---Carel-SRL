...
# ahora sí importar
from services.matchers.parser_fleb import parse_fleb
...
def test_fleb_parser():
    """Prueba el parsing del CSV de Efepe"""
    
    # paths
    input_path = os.path.join(BASE_DIR, "ddbb", "flebSinParsear.csv")
    output_path = os.path.join(BASE_DIR, "ddbb", "flebParseado.csv")
    
    # Validar que exista el archivo
    if not Path(input_path).exists():
        logger.error(f"❌ Archivo no encontrado: {input_path}")
        logger.info("   Por favor, ejecuta primero: python testing/test_fleb.py")
        return

    # Crear directorio de salida si no existe
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    logger.info("\n🚀 Iniciando Fleb Parser...\n")
    
    # Ejecutar parsing
    df = parse_fleb(input_path, output_path)
